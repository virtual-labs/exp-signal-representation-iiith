...
In an analogous way, we can define the notion of dot product (and hence orthogonality) for signals. Signals can be thought of as vectors with infinite dimension. For continuous-time signals $ \mathbf{x}\_1 (t) $ and $ \mathbf{x}\_2 (t) $ , their dot product is defined as
...
As an example, consider the signals $ \mathbf{x}\_1 (t) = \sin(2\pi t) $ and $ \mathbf{x}\_2 (t) = \cos (4\pi t) $. We can verify that their dot product is zero as seen by the product signal and the shaded areas in Fig.1 and Fig.2 respectively.
...
The notion of dot product and orthogonality can be extended to complex signals. If $ \mathbf{x}\_1 (t) $ and $ \mathbf{x}\_2 (t) $ are complex-valued signals, their dot product is defined as
...
For finite $ N $-length signals, dot product is defined as

$$ \langle\mathbf{x}_1 [n],\mathbf{x}\_2 [n]\rangle ~= \sum_{n=0}^{n=N-1} \mathbf{x}\_1 [n] \mathbf{x}\_2 [n] $$

### Fourier Series

As in vectors, orthogonal signals are extensively used in representation of signals. They often form the building blocks for various signal representations. For example, in trigonometric Fourier series analysis, a periodic signal $ \mathbf{x}(t) $ is represented using sinusoids as follows:

$$ \mathbf{x}(t) = a*0 + \sum*{k=1}^{\infty} a_k \cos(2\pi k f_0 t) + b_k \sin(2\pi k f_0 t),$$

where $ T = \frac{1}{f*0} $ is the period of $ \mathbf{x}(t) $. The set of signals $ \{1, \cos(2\pi k f_0 t), \sin(2\pi k f_0 t)\}*{k = 1,2,...} $ are the building blocks in Fourier series representation of periodic signals, where $ 1 $ denotes the constant signal. All the signals in this set have the common period of $ T $. We can verify that any two signals in this set are orthogonal.
...
As an example, consider the [Haar mother wavelet](https://en.wikipedia.org/wiki/Haar_wavelet#:~:text=In%20mathematics%2C%20the%20Haar%20wavelet,terms%20of%20an%20orthonormal%20basis.) $ \psi(t) $ and its scaled and translated version with scale factor $ n $ and shift $ k $ given by,
...
<p align="center"><img src="./images/p3_haar_1.png" alt="drawing" width="400"/<em> <p align="center"> Fig.3: Haar mother wavelet</em>

We can verify that the scaled and shift versions of the Haar wavelet are orthogonal to each other. As a special case, orthogonality of the wavelets $ \psi*{0,0}(t) $ and $ \psi*{1,0}(t) $ can be seen below.
...
Finding the representation coefficients becomes easy when a signal is decomposed into a set of orthogonal signals. Consider the trigonometric Fourier series representation given above. We can find the coefficients $ \{a*0, a_k, b_k\}*{k=1,2...} $ by computing the dot product of the given periodic signal $ \mathbf{x}(t) $ with the signal associated with each of these components. Thus,

$$ a*0 ~= \frac{1}{T} \int*{0}^{T} \mathbf{x}(t)~ dt $$

$$ a*k ~= \frac{1}{T} \int*{0}^{T} \mathbf{x}(t) \cos(2\pi k f_0 t)~ dt $$

$$ b*k ~= \frac{1}{T} \int*{0}^{T} \mathbf{x}(t) \sin(2\pi k f_0 t)~ dt $$
