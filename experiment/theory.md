...
We can verify that the scaled and shift versions of the Haar wavelet are orthogonal to each other. As a special case, orthogonality of the wavelets $ \psi_{0,0}(t) $ and $ \psi_{1,0}(t) $ can be seen below.
...
## Advantages of orthogonality ##
 Finding the representation coefficients becomes easy when a signal is decomposed into a set of orthogonal signals. Consider the trigonometric Fourier series representation given above. We can find the coefficients $ \{a_0, a_k, b_k\}_{k=1,2...} $ by computing the dot product of the given periodic signal $ \mathbf{x}(t) $ with the signal associated with each of these components. Thus,

$$ a_0 ~= \frac{1}{T} \int_{0}^{T} \mathbf{x}(t)~ dt $$

$$ a_k ~= \frac{1}{T} \int_{0}^{T} \mathbf{x}(t) \cos(2\pi k f_0 t)~ dt $$

$$ b_k ~= \frac{1}{T} \int_{0}^{T} \mathbf{x}(t) \sin(2\pi k f_0 t)~ dt $$
